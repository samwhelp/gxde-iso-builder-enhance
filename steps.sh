...
gxde_build_os_bootstrap () {
	...
	if [[ "${build_arch}" == "loong64" ]]; then
		...
		util_error_echo
		util_error_echo debootstrap --no-check-gpg --include="${build_include}" --keyring="${build_keyring_for_loong64}" --arch="${build_arch}" "${build_suite}" "${rootfs}" "${package_repo_url_for_loong64}"
		util_error_echo
		debootstrap --no-check-gpg --include="${build_include}" --keyring="${build_keyring_for_loong64}" --arch="${build_arch}" "${build_suite}" "${rootfs}" "${package_repo_url_for_loong64}"
...
gxde_build_os_package_install_input_method () {
	...
	util_error_echo
	util_error_echo core_chroot_package_control install im-config fcitx5 fcitx5-frontend-all fcitx5-chinese-addons fcitx5-chewing fcitx5-pinyin -y
	util_error_echo
	core_chroot_package_control install im-config fcitx5 fcitx5-frontend-all fcitx5-chinese-addons fcitx5-chewing fcitx5-pinyin -y
